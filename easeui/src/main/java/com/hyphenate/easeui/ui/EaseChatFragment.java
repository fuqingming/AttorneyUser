...
import com.hyphenate.easeui.utils.OnTaskSuccessComplete;
...
public class EaseChatFragment extends EaseBaseFragment implements EMMessageListener {
    ...
    protected boolean isKefu;
    protected boolean isChat;
    protected  String strChatTime;
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        isKefu = fragmentArgs.getBoolean(EaseConstant.CHATTYPE_KEFU,false);
        isChat = fragmentArgs.getBoolean(EaseConstant.CHATTYPE_NO_CHAT,false);
        strChatTime = fragmentArgs.getString(EaseConstant.CHATTYPE_CHAT_TIME);
...
//        messageList.setAvatarShape(1);
    ...
    protected void setUpView() {
        ...
        if("kefu".equals(toChatUsername))
        {
            titleBar.setTitle("上海律界只能科技有限公司");
        }
        else
        {
            titleBar.setTitle(toChatUsername);
            if(!"0".equals(strChatTime))
            {
                titleBar.setNotifyTime(getActivity(),Integer.parseInt(strChatTime), new OnTaskSuccessComplete()
                {
                    @Override
                    public void onSuccess(Object obj)
                    {
                        Utils.showCancleOrderDialog(getActivity(), new OnTaskSuccessComplete()
                        {
                            @Override
                            public void onSuccess(Object obj)
                            {
                                boolean isFlag = (boolean)obj;
                                if(isFlag)
                                {
    ...
    public void onMessageReceived(List<EMMessage> messages) {
        for (EMMessage message : messages) {
            ...
            if (message.getChatType() == ChatType.GroupChat || message.getChatType() == ChatType.ChatRoom) {
                username = message.getTo().toLowerCase();
            } else {
                // single chat message
                username = message.getFrom().toLowerCase();
            }

            // if the message is for current conversation
            String strUserName = toChatUsername.toLowerCase();
            if (username.equals(strUserName) || message.getTo().equals(strUserName) || message.conversationId().equals(strUserName)) {
                messageList.refreshSelectLast();
