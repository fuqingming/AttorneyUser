...
public class ForgetPwdActivity extends BaseAppCompatActivity
{
    ...
    @OnClick({R.id.tv_register_getCode,R.id.tv_register_confirm,R.id.rl_other_type})
    public void onViewClick(View view)
    {
        switch (view.getId()) {
            ...
            case R.id.tv_register_confirm://注册
                ...
                break;
            case R.id.rl_other_type://其他方式找回密码
                Intent intent = new Intent(this,ForgetPwdEmailActivity.class);
    ...
    {
        ApiStores.smsSend(m_strPhone, Const.YzmCode.yzm_code_type_forget_pwd, new HttpCallback<ResponseBaseBean>()
        {
            @Override
            public void OnSuccess(final ResponseBaseBean response)
            {
                if(response.getStatus() == HttpCode.HTTPCODE_SUCCESS)
                {
                    ...
                    m_myCount = new SmsSendCounter(ForgetPwdActivity.this,m_tvCode, RESEND_VERIFY_CODE_SECOND * 1000, 1000);
                    m_myCount.start();
                ...
                {
                    Utils.showToast(ForgetPwdActivity.this,response.getResult());
                }
            }
            ...
            {
                AlertUtils.MessageAlertShow(ForgetPwdActivity.this, "错误", message);
            }
    ...
    {
        ApiStores.forgetPass(m_strPhone, m_strPassword,m_strVerifyNumber, new HttpCallback<ResponseBaseBean>()
        {
            @Override
            public void OnSuccess(final ResponseBaseBean response)
            {
